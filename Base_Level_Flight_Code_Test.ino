// BASE-LEVEL FLIGHT CODE TEST - STUDENT VERSION
...
void setup() {
  ...
  Serial.println("Beginning Base-Level Flight Code Test");

  // Initialize the digital pin as an outputs
  ...
  // File Management
  if (SD.exists("Base-Level_Flight_Code_Test.csv")) {
    // File exists: append log marker
    dataFile = SD.open("Base-Level_Flight_Code_Test.csv", FILE_WRITE);
    if (dataFile) {
      dataFile.println("# --- NEW LOG STARTED ---"); // Marks new log starting 
      dataFile.close(); // Closes file
      Serial.println("Existing data file found. Appending…"); // Sends message to serial
    }
  } else {
    ...
    dataFile.close();
      ...
      Serial.println("Error opening Base-Level_Flight_Code_Test.csv");  // Print message if file couldn't be created
...
void loop() {
  ...
  if (millis() - startTime >= 1200000) {     // Check if 20 mins in milliseconds have passed
 ...
 // Log data to the file if the SD file can be found
  if (sdAvailable()) {
    ...
    dataFile = SD.open("Base-Level_Flight_Code_Test.csv", FILE_WRITE); // Reopen file in prep to write to it
    ...
    Serial.println("Error writing to Base-Level_Flight_Code_Test.csv");
    digitalWrite(greenPin, LOW); // Turn off green LED
    digitalWrite(redPin, HIGH); // Turn on red LED
